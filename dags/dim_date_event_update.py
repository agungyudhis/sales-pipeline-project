...
def _fetch_data(task_instance: TaskInstance | None = None):
    ...
    response = requests.get(
        f"https://hari-libur-api.vercel.app/api?year={task_instance.execution_date.year}"
    )
    data = pl.DataFrame(response.json()).with_columns(
        pl.col("event_date").map_elements(
            lambda x: x.replace("-", ""), return_dtype=pl.String
        ),
        pl.col("is_national_holiday").map_elements(
            lambda x: "Holiday" if x else "Non-Holiday", return_dtype=pl.String
        ),
    )
...
with DAG(
    ...
    description="Fetch national event or holiday data every six months.",
    ...
    upsert_data = SQLExecuteQueryOperator(
        task_id="event_upsert", sql="dim_date_event_update.sql", conn_id="postgres-dw"
    )
