...
def upload_to_minio(bucket, folder, data, filename):
    ...
    with io.StringIO() as file_buffer:
        ...
        status = response_upload.get(
            "ResponseMetadata", {}).get("HTTPStatusCode")

        if status == 200:
            print(
                f"Successful S3 put_object response. Status - {status}")
        else:
            print(
                f"Failed to upload file to MinIO. Status - {status}")
            response_upload.raise_for_status()

def _extract_orders(task_instance: TaskInstance | None = None):
    ...
    if data['order_count']:
        upload_to_minio('raw', 'orders', data, START_DATE.strftime(r"%Y%m%d%H%M") + ".json")
        return 'load_orders'
    else:
        return 'skip_empty'

def _extract_web_traffic(task_instance: TaskInstance | None = None):
    ...
    if data['visitor_count']:
        upload_to_minio('raw', 'traffic', data, START_DATE.strftime(r"%Y%m%d%H%M") + ".json")
        return 'load_web_traffic'
    else:
        return 'skip_empty'

def s3_decorator(bucket, folder, minio_credentials_variable, pg_database, pg_schema, pg_table):
    def inner(func):
        def wrapper(*args, **kwargs):
            ...
            if len(response['Contents']):
                ...
                for file in sorted(response['Contents'], key=lambda d: d['Key']):
                    if file['Key'][-5:] == '.json':
                        response_object = s3.get_object(
                            Bucket='raw', Key=file['Key'])
                        data_list = json.load(io.BytesIO(response_object['Body'].read()))
                ...
                engine = sqlalchemy.create_engine(f'postgresql://{DB_USER}:{quote_plus(DB_PASSWORD)}@{DB_HOST}:{DB_PORT}/{pg_database}')
                
                # Connect to db
                with engine.connect() as connection:
                    all_data.write_database(f'{pg_schema}.{pg_table}', connection, if_table_exists='replace')

                delete_response = s3.delete_objects(
                    Bucket=bucket,
                    Delete={
                        'Objects': [{'Key': item['Key']} for item in response['Contents'] if item['Key'][-5:] == '.json']
                    }
                )
                delete_status = delete_response.get(
                    "ResponseMetadata", {}).get("HTTPStatusCode")
...
def _load_web_traffic(data_list):
    ...
    data = (
        pl.DataFrame([Visitors(**item).model_dump() for item in data_list['visitors']])
    )
...
with DAG(
    ...
    skip_empty = EmptyOperator(
        task_id='skip_empty_data'
    )
